...
//        params.put("pwd", pwd);
    ...
    public void onSucceed(String requestUrl, String response) {
        ...
        if (Constant.SUCCEED_CODE == resResponseMessage.getRet()) {
            String token = SharedPrefsUtil.getValue("InitialInfo", "AccessToken", "");
            if (token.equals(resResponseMessage.getData().getToken())) {
                startActivity(new Intent(this, FacilityListActivity.class));
                finish();
            } else {
                SharedPrefsUtil.putValue("InitialInfo", "AccessToken", resResponseMessage.getData().getToken());
                startActivity(new Intent(this, FacilityListActivity.class));
                finish();
            }
        } else {
            ToastUtilsBase.showToastCenter(this, resResponseMessage.getMsg());
        }
    }
...
//                    return;
//                }
                login(userPhoneEt.getText().toString(), pwdEt.getText().toString());
                break;
